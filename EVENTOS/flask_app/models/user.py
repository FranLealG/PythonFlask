...
class User:
    ...
    def get_by_email(cls, form):
        ...
        result = connectToMySQL('events').query_db(query, form) #retorna una lista de diccionarios 
    ...
    def get_by_name(cls, form):
        ...
        result = connectToMySQL('events').query_db(query, form)
        return cls(result[0])
    ...
    def validate_user(form):
        ...
        if len(form['first_name']) < 2:
            flash('Tu nombre debe tener al menos 2 caracteres', 'first_name') #flash(mensaje, categoria)
            is_valid =  False
        
        if len(form['last_name']) < 2:
            flash('Tu apellido debe tener al menos 2 caracteres', 'last_name')
            is_valid =  False

        if len(form['password']) < 6:
            flash('Tu contraseña debe tener al menos 6 caracteres', 'password')
            is_valid =  False
        ...
        if form['password'] != form['confirm-pass']:
            flash('Las constraseñas no son iguales!', 'pass_confirm')
            is_valid = False
